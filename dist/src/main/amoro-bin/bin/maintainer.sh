...
export CLASSPATH=$AMORO_CONF_DIR/maintainer:$LIB_PATH/:$(find $LIB_PATH/ -type f -name "*.jar" | paste -sd':' -)
...
if [ -z "$OPTIMIZER_LOG_DIR_NAME" ]; then
  OPTIMIZER_LOG_DIR=$AMORO_LOG_DIR/maintainer-$(date +%s)
else
  OPTIMIZER_LOG_DIR=$AMORO_LOG_DIR/$OPTIMIZER_LOG_DIR_NAME
fi
STDERR_LOG=${OPTIMIZER_LOG_DIR}/maintainer.log.err
...
RUN_SERVER="org.apache.amoro.maintainer.StandaloneMaintainer"
CMDS="$JAVA_RUN $JAVA_OPTS $RUN_SERVER $ARGS"

start() {
  nohup $CMDS > /dev/null 2>"$STDERR_LOG" &
  echo "Amoro Maintainer started in background with PID $!"
}
